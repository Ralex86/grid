...
class Slider extends Component<OwnProps, State> {
  ...
  panResponder = PanResponder.create({
    ...
    onPanResponderMove: (evt, gestureState) => {
      Animated.event([null, {dx: this.state.translate}])(evt, gestureState);
    },
  ...
  endGesture = (evt: any, gestureState: any) => {
    ...
    const destIndex = userIsSwipingLeft ? currentIndex + 1 : currentIndex - 1;
  ...
  requestSlideIndex = (currentIndex: number, destIndex: number) => {
    if (destIndex > this.slideCount - 1) {
      this.animate(
        this.slideCount - 1,
        this.setValueForAnimatedSpring(currentIndex, this.slideCount - 1),
      );
    } else if (destIndex < 0) {
